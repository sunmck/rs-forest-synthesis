###################################################################
##
## Aggregate Data to Polygons (optionally parallel implementation)
##
###################################################################
...
# TODO: Toggle for parallelization. choose between TRUE or FALSE
# for hexagons TRUE is highly recommended as it decreases computational time significantly
use_parallel <- FALSE  

# TODO: set polytype, choose between "federal", "forest_landscapes", "forest_districts"
poly_type <- "forest_landscapes"
...
# merge forest_districts by attribute bez_wg_bu
merged_forest_district_3035 <- forest_district_3035 %>%
  group_by(bez_wg_bu) %>% 
  summarize(geometry = st_union(geometry), .groups = "drop")


if(poly_type == "forest_districts"){
  poly_names <- merged_forest_district_3035$bez_wg_bu
} else if(poly_type == "federal"){
  poly_names <- federal_3035$NAME_1
} else if(poly_type == "forest_landscapes"){
  poly_names <- forest_landscape_3035$gl_name
}

fulltab <- data.frame(matrix(NA, nrow = 0, ncol = 92))
colnames(fulltab) <- c("year", "poly_name", "district", "federal", "forst_landscape", "forest_district", "forest_area_ha", "pine_perc", "spruce_perc", "beech_perc", "oak_perc", "other_treespecies_perc", "dec_perc", "con_perc",
...
process_polygon <- function(poly_name) {
  
  gc()
  # get pixels per polygon
  if(poly_type == "forest_districts"){
    sub_3035 <- subset(merged_forest_district_3035,merged_forest_district_3035$bez_wg_bu==poly_name)
  } else if(poly_type == "federal"){
    sub_3035 <- subset(federal_3035,federal_3035$NAME_1==poly_name)
  } else if(poly_type == "forest_landscapes"){
    sub_3035 <- subset(forest_landscape_3035,forest_landscape_3035$gl_name==poly_name)
  }
  ...
  for(j in 1:7){
    ...
    hex_results[j, ] <- c(
      2016 + j, poly_name, district, federal, fl, fd, forest_area_ha, pine_per, spruce_per, beech_per, oak_per, unclass_per, dec_per, con_per,
...
if (use_parallel) {
  results_list <- mclapply(poly_names, process_polygon, mc.cores = num_cores)
} else {
  results_list <- lapply(poly_names, process_polygon)
}

fulltab <- do.call(rbind, results_list)
write.csv(fulltab, paste0("./data/results/forestdata_fulltab_", poly_type, "_reproj.csv"), row.names = FALSE)


## write data to geopackage
# 
# fulltab <- read.csv("./data/forestdata_fulltab_federal_reproj.csv")
# federal_3035 <- federal_3035 %>% select("NAME_1")
# shp_re_full <- merge(hex_3035, fulltab, by.x="NAME_1", by.y="poly_name", all.x=T, sort=F)
# 
...
# st_write(fulltab_2017, "./data/results/fulltab_federal_2017_reproj.gpkg", overwrite=T)
# st_write(fulltab_2018, "./data/results/fulltab_federal_2018_reproj.gpkg", overwrite=T)
# st_write(fulltab_2019, "./data/results/fulltab_federal_2019_reproj.gpkg", overwrite=T)
# st_write(fulltab_2020, "./data/results/fulltab_federal_2020_reproj.gpkg", overwrite=T)
# st_write(fulltab_2021, "./data/results/fulltab_federal_2021_reproj.gpkg", overwrite=T)
# st_write(fulltab_2022, "./data/results/fulltab_federal_2022_reproj.gpkg", overwrite=T)
# st_write(fulltab_2023, "./data/results/fulltab_federal_2023_reproj.gpkg", overwrite=T)
